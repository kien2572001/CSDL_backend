  ...
  return new Promise(async (resolve, reject) => {
    try {
      let data = await db.execute(
        `insert into order_item  (orderId,pid,price,quantity)
                values (?,?,?,?);`,
        [item.orderId, item.pid, item.price, item.quantity]
      );

      let subtractQuantity = await db.execute(
...
let findOrderById = (id) => {
  return new Promise(async (resolve, reject) => {
    try {
      let data = await db.execute(
        'select  DISTINCT * from `order`inner join order_item on `order`.orderId = order_item.orderId where `order`.orderId = "' +
          id +
          '";'
      );
      resolve(data);
...
let deleteProductById = async (pid) => {
  return new Promise(async (resolve, reject) => {
    try {
      let data1 = await db.execute(
        "delete from product_category where pid = ?;",
        [pid]
      );
...
let getProductByName = (name) => {
  return new Promise(async (resolve, reject) => {
    try {
      let data = await db.execute(
        'select * from product where title like "%' + name + '%"'
      );
      resolve(data);
    } catch (error) {
      reject(error);
